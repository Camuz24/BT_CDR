    ...
    while(!stopThread){
        ...
        if(!stopSend){
            ...
            if(counter_low_priority == wait_cycles){
                ...
                LEFT_PID_PERCENTAGE, RIGHT_PID_PERCENTAGE, 
                LEFT_CURRENT_PERCENTAGE, RIGHT_CURRENT_PERCENTAGE,
                QUAD_L, GLU_L, HAM_L, GAS_L,
                QUAD_R, GLU_R, HAM_R, GAS_R,
                HEART_RATE, STIMULATOR_FREQUENCY, STIMULATOR_PULSEWIDTH, 
                LEFT_POWER, RIGHT_POWER, TOTAL_POWER, SINGLE_TARGET_POWER, GEAR, CURRENT_OR_TARGET};       // lasciare CURRENT_OR_TARGET come ultimo valore!!!
                ...
                to_string((int) shmem->data->trg_cad), to_string(shmem->data->left_pid_percentage),
                to_string(shmem->data->right_pid_percentage), to_string(shmem->data->left_current_percentage),
                to_string(shmem->data->right_current_percentage), to_string((int) shmem->data->theorCurrentsL[0]),
                ...
                to_string(shmem->data->total_power), to_string((int)shmem->data->single_target_power), 
